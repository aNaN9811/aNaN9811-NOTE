...
......
    ...
    public void onClick(View v){
        switch (v.getId()){
            case R.id.button:
                //通过 AlertDialog.Builder 创建一个 AlertDialog 的实例，然后为这个对话框设置标题内容、可否用 Back 键关闭对话框等属性
...
RelativeLayout 中海油另外一组相对于控件进行定位的属性，android:layout_alignLegt 表示一个控件的左边缘和另一个控件的左边缘对齐，android:layout_alignRight 表示让一个控件的右边缘和另一个控件的右边缘对齐
...
在这种布局中，我么你可以不再使用 wrap_content、match_parent 等方式来指定控件的大小，而是允许直接指定控件在布局中所占的比例，这样就可以轻松实现平分布局甚至是任意比例分割布局的效果了
...
最后被忘了在 MainActivity 中将系统自带的标题隐藏掉
...
把 ListViewTest 项目中的图片还有 Fruit 类中和 fruit_item.xml 复制过来，接下来为 RecyclerView 准备一个适配器，新建 FruitAdapter 类
...
Nine-Patch 图片是一种被特殊处理过的 png 图片，能够制定哪些区域可以被拉伸、那些区域不可以
...
下边框和有边框小黑点表示内容会被放置的区域

用鼠标在图片的边缘拖动就可以进行绘制，按住 shift 键拖动可进行擦除
        ...
        send.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                if(!"".equals(content)){
                    ...
                    adapter.notifyItemInserted(msgList.size() - 1);//当有新消息时，刷新 RecyclerView 中的而现实
